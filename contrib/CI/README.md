...
Project name: pampa-master or pampa-dev

Description: Build PAMPA for Win, Mac, Linux
...
In the build step paste script:
    ...
    wget https://raw.githubusercontent.com/PampaCoin/PampaCoin/master/contrib/CI/jenkins.sh -O $WORKSPACE/jenkins.sh
    chmod +x $WORKSPACE/jenkins.sh
    export RELEASEDIR=/var/www/pampa
...
Create directory `/var/www/pampa` and make Jenkins owner of that directory

    sudo mkdir /var/www/pampa
    sudo chown jenkins /var/www/pampa
